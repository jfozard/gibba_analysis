...
base_data_dir = base_dir + 'bladder_new/data/'
...
class MultiSegment(Process):
    ...
    def exec_process(self, input_files, output_files, params, signal_name):
        ...
        if os.path.exists(input_files[0]+'.ply'):
            ...
            for seed in range(5):
                script_fn = base_script_dir + 'multisegment_script.txt'
...
#            print subprocess.check_output([slurm_run, signal_name_slurm, 'singularity', 'run', '--app', 'image',
#                                           base_container_dir+'segment2.img', '--ip_scalebar', '100',
#                                           '-s', edited_script, '-i', output_file_slurm, input_file_slurm])
    ...
    def get_slurm_command(self, input_files, output_files, params):
        ...
        if os.path.exists(input_files[0]+'.ply.gz'):
            ...
            return ['singularity', 'run', '--app', 'image',
                                           ...
                                           '-s', edited_script, '-i', output_file_slurm, input_file_slurm, '\n'
            'singularity', 'run', '--app', 'image',
                                           base_container_dir+'segment2.img',
                                           '--ip_scalebar', '100', '--ip', 'aniso', '--ip_range', '0,1', '--ip_colmap', 'new_jet',
                                           '--i_matrix', '1_0_0_0_0_0_1_0_0_-1_0_0_0_0_0_1', 
                                           '-s', edited_script, '-i', output_file_chin_slurm, input_file_slurm]
...
#            print ['python', surfacespm_local, '-i',output_file, input_file]
...
def main():
    process_list = [Enhance(), Project(), MultiSegment(), MultiCombine(), Clean(), Clip(),  Clean2(), ImageLabels(), ImageRelArea(), ImageAbsLogArea(), ImageAniso(), ImageRange(), AnalyseCells(), SummaryBladder(), AnalyseAll(), MaxProj(), SummaryTable(), SummaryGraph()]
