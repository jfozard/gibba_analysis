...
def summary_table(output_filename, output_dir, input_filenames, DAI, HAI, size_classes, lengths, names):
    ...
    f.write('<tr>\n')
    for heading in ['name', 'DAI', 'HAI', 'trap_length', 'class', 'maxproj', 'labels', 'relarea', 'abslogarea', 'aniso', 'aniso_chin']:
    ...
    for i, (fn,dai0, hai0, c,length, n) in enumerate(zip(input_filenames, DAI, HAI, size_classes, lengths, names)):
        ...
        f.write('<tr style="background-color:'+bg_color[c][i%2]+'"> <td> <a href="'+link_fn+'"> '+n[:40]+'</a></td><td>'+str(dai0)+'</td><td>'+str(hai0)+'</td><td>' +str(length)+'</td><td>'+c+'</td>')
